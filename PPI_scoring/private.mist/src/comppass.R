...
Comppass.WDv2 = function(stats_tab, speci_tab, repro_tab, num_reps, normalized=F, WD_T=1){ ## 95% score as normalization bar
  ...
  for(i in 1:nrow(tmp)){
    for(j in 1:ncol(tmp)){ # per bait
      tmp[i,j] = tmp[i,j] * (w[i] * inv_freq[i] ^ (1 + (repro_tab[i,j]-1)/num_reps[j])) #divide exponent by number of replicates
     }
  }
  WD = sqrt(tmp)
  if(normalized){
...
Comppass.main = function(data_file, output_file, resampling=F){
  ...
  # remove 0 abundance bait/prey pairs
  summary = summary[summary$Abundance != 0,]
