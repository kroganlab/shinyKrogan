...
mist.getM3D_normalized <- function(x){
  # remove anything with a length of 0. Also must consider the case where there is not input (NA's from the MaxQuant data)
  if( any( (x[,3]<1) | is.na(x[,3]) ) ) {
    idx = which( (x[,3]<1) | is.na(x[,3]) )
    #names(x)[grep("[1-9]", x[idx,-c(1:4)])]
    cat(paste("  WARNING: Prey '", x[idx,1], "' has a recorded length < 1. Removing entry prey from scoring set.\n",sep=""))
    x = x[-idx,]
  }
...
mist.getMetrics <- function(x, info){
	...
	for( i in sort(unique(info$Bait))){
    ...
    if(length(bidx)>1){
      y <- y/apply(y, 1, sum)
		...
		if(length(bidx)<2){ #if there's only one replicate for that bait
      #y <- sum(y)  # !!!! This ruins reproducibility if the first bait has no replicates. Causes reproducibility to be a 1x1 and throws everything off.
		                #       This will be assigned a reproducibility scor of 0 further down anyways
		  y <- y
		}else{
      y <- rowSums(y)
		}
...
mist.getSpecificity <- function(abundance, info){
  ...
  pb = txtProgressBar(min=1, max=dim(abundance)[2], style=3)
  for( i in 1:dim(abundance)[2]){
    ...
    baits_to_exclude = baits_to_exclude[!baits_to_exclude %in% bait]
    #baits_to_include = setdiff(colnames(abundance),baits_to_exclude)
    baits_to_include = colnames(abundance)[!colnames(abundance) %in% baits_to_exclude]
...
mist.main <- function(matrix_file, weights='fixed', w_R=0.30853, w_A=0.00596, w_S=0.68551, training_file, training_steps=0.05, true_negative_size, plot_roc){
  dat <- read.delim(matrix_file, sep="\t", header=TRUE, stringsAsFactors=FALSE)
  
  # Check for baits with name "NA". If there are, make sure they arent turned into actual <NA> values
  nas <- is.na(dat)
  if(any(nas)){
    dat[nas] = "NA"
    cat("\t  NA's found in bait names. Assuming the baits are actually named 'NA'.\n")
  }
...
#   write.table(metrics, output_file, row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t" )
  ...
  if(weights == 'fixed'){
    ...
    training_weights = mist.train.main(metrics, training_set, output_file, training_steps, true_negative_size)
    cat(sprintf("\tWEIGHTS BASED ON TRAINING SET:\n\t  REPRODUCIBILITY: %s\n\t  ABUNDANCE: %s\n\t  SPECIFICITY: %s\n\t  THRESHHOLD: %s\n",training_weights$R, training_weights$A, training_weights$S, training_weights$threshold))
    ...
    stop(sprintf('unrecognized MIST option: %s',weights))
  }
