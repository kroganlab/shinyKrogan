...
resample.main = function(termsNscores, out_file, termid, heat_style="Q", FILTER_HICONF=T, AGGREGATE_FUN=mean, SCORE_T=0.7, score_name="MIST", ITERATIONS, P_T, BREAKS, LABELS, TERM_SIZE_T=Inf){
  term_src = data.frame(termsNscores, stringsAsFactors=F)
  ## select only list of proteins that are on the interactome
  #if(FILTER_HICONF){            #REMOVED BECAUSE WE WANT TO CALCULATE FOR ALL TERMS
  ...
  tmp = unique(term_src[,c("BAIT","TERM","uniprot_id",score_name)])
  ...
  term_groups = term_groups[term_groups$count > 1 & term_groups$count <= TERM_SIZE_T, ] ## remove singletons & too large groups
...
overRepresented.main <- function(score_file, output_dir, uniprot_dir="/files/", score_name="MIST", prey_name="Prey", bait_name="Bait", score_threshold=0.7, TERM_SIZE_T=Inf){
  cat("\tMapping GO Terms...\n")
  scores =tryCatch(data.table(read.delim(score_file, sep="\t", stringsAsFactors=F)), error=function(e){ data.table(score_file) })
  ...
  write.table(unique(goCC), out_file, row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")
  ...
  write.table(unique(goBP), out_file, row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")
  ...
  write.table(unique(goMF), out_file, row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")
  ...
  write.table(unique(pfam), out_file, row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")
  ...
  write.table(unique(kegg), out_file, row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")
