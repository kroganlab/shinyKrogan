...
qc.getIpToBaits = function(input_matrix){
  bait_row_idx = 1
  data_col_idx = 5
  df = data.frame(colnames(input_matrix)[data_col_idx:ncol(input_matrix)],t(input_matrix[bait_row_idx,data_col_idx:ncol(input_matrix)]), 0, stringsAsFactors=F)
  colnames(df) = c("ip","bait","replicate")
  bait_reps = aggregate( ip ~ bait, FUN=length, data=df)
  
  # get's the number of replicates per bait?
...
qc.clusterHeatmap = function(data_matrix, output_file, ip_baits, font_scale){
  ...
  write.table(cor_matrix, gsub(".pdf",'_correlation.txt',output_file), quote=F, row.names=T, col.names=T, sep='\t')
  ...
  #Now look for cellular neighborhood clustering
#   data_matrix_w_names = t(data_matrix)
#   cor_matrix = cor(data_matrix_w_names, use="pairwise.complete.obs", method="pearson")
#   color_scale = colorRampPalette(brewer.pal(7,"Blues"))(9)
#   output_file = gsub(".pdf","_neighborhood.pdf", output_file)
#   pheatmap(cor_matrix, cluster_rows=T, cluster_cols=T, scale="none",fontsize_row=font_scale,fontsize_col=font_scale, cellwidth=font_scale, cellheight=font_scale, border_color=NA, filename=output_file, color=color_scale, breaks=1:10/10, clustering_distance_rows="correlation", clustering_distance_cols="correlation", treeheight_row=0, treeheight_col=0) 
}

# Plots the number of proteins per sample after removing the sub-ranks
qc.ipDists = function(data_matrix, ip_baits, baseName){
  ...
  p = ggplot(data_long, aes(x=ip)) + geom_bar(stat='count') + facet_wrap(facets= ~bait, scales='free_x', ncol=10) + theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
qc.main = function(matrix_file, font_scale, cluster=T, ip_dists=T){
  ...
  # There should be no NA's in this initial data. If there is, it is presumed that the Bait name is "NA". Converting here. 
  nas = is.na(ip_matrix)
  if(any(nas)){
    cat("\t WARNING : NA's found in list of baits. Assuming bait name is 'NA'.\n")
    ip_matrix[nas] = "NA"
  }
